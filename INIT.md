...
- [x] **M5 进行中（2026-09-09，部分完成）**：① systemd 托管 gateway daemon（`bsk-gateway.service`，--gateway --listen 0.0.0.0 --agent-port 52901 --双token --4 CIDR，Restart=always 5s，kill -9 自愈实测 PID 轮换）；② 本机三入口验证（127.0.0.1 / Tailscale IP / 错误 token 拒）；③ 三件套文档已写并推送（docs/ops/systemd-gateway.md、firewall-ufw.md、rollback.md，commit 329fddb）；④ README 构建路径纠正 .output→dist（67eae60）；⚠️ us01 跨机 TCP 不通 = us01 网络环境问题（DERP 香港中继 hkg + exit-node nft 规则干扰 + UDP 直连未建立），非 bsk 代码问题，改由 Mac 第二浏览器主机承担真跨机验证；⏳ 待办：Mac 装扩展（用户配合）+ 真跨机 soak + Windows 真机
- [ ] M5 剩余：Mac 第二浏览器主机真机（米罗装扩展配 URL/token）→ kill daemon 重连 soak → Windows 真机（米罗方便时）
